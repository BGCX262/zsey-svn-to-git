...
function orderSubmit(){
	...
	if($.trim(id)!=""){
		if(window.confirm('你确定要提交吗？')){
			location.href="<c:url value='/sell/order/sell-order/submit.jhtml'/>?id="+id+"&rnd="+Math.random();
		}
	}
}
</script>
...
<div class="main">
	...
	<form name="form1" action="<c:url value='/sell/order/sell-order/index.jhtml'/>" method="post">
    <div class="frmBar mT5">
		...
		<input type="button" class="btn" value="添加" onclick="location.href='<c:url value='/sell/order/sell-order/edit.jhtml'/>'"/>&nbsp;&nbsp;
    ...
    <div class="ntable_mid mT5">
		<display:table name="pageList" id="vo" class="nm_table" style="width:100%" cellspacing="0" requestURI="index.jhtml">
			...
			<display:column title="操作" style="width:150px;">
				<span class="caozuo">
					<a href="javascript:void(null);" onclick="openDialog('details.jhtml?id=${vo.id}','${vo.code }订单详情');" class="show">详情</a>
			...
			<display:setProperty name="paging.banner.item_name">订单</display:setProperty>
			<display:setProperty name="paging.banner.items_name">订单</display:setProperty>
		</display:table>
	</div>
