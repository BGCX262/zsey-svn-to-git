...
function countCostTots(){
	...
	$(":input[type='text'][name='vo.nopay']").val((payment-parseFloat(payfor)).toFixed(2));
}
</script>
</head>
<body>
...
<form name="form1" action="<c:url value='/sell/order/sell-order/save.jhtml' />" method="post" onsubmit="return Validator.Validate(this,3);" enctype="multipart/form-data">
    <fieldset class="tableBox">
    	...
    	<table width="100%" class="nt_table">
			...
			<tr>
                ...
                <td><input name="vo.deliver" type="text" class="ntext_01" value="${vo.deliver }" size="35" readonly="readonly" onclick="WdatePicker();" /></td>
            </tr>
            <tr>
                ...
                <td>
                	<select name="vo.grade" dataType="Require" msg="请选择订单紧急度">
						<option value="" <c:if test="${empty vo.grade }">selected="selected"</c:if>>---请选择---</option>
						<c:forEach var="grade" items="${grades}">
							<option value="${grade}" <c:if test="${vo.grade==grade }">selected="selected"</c:if>>${grade.text }</option>
						</c:forEach>
					</select>
                </td>
	...
	<div class="nbot_control mT10">
		...
		<input type="button" class="formBtn02" value="返 回" onclick="location.href='<c:url value='/sell/order/sell-order/index.jhtml' />';" />
    </div>
</form>
</div>
</body>
