...
<div class="main">
    ...
    <div class="frmBar mT5">
		...
		型号: <input class="input" type="text" name="vo.model" value="${vo.model}"/>&nbsp;&nbsp;
    ...
    <div class="ntable_mid mT5">
			<display:table name="pageList" id="vo" class="nm_table" style="width:100%" cellspacing="0" requestURI="index.jhtml">
				...
				<display:column title="型号">${vo.model}</display:column>
				<display:column title="规格">${vo.spec}</display:column>
				<display:column title="单位">${vo.unit.name}</display:column>
				<display:column title="价格"><fmt:parseNumber type="number" pattern="##0.00">${vo.price}</fmt:parseNumber></display:column>
