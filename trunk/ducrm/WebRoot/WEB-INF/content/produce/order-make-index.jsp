...
<div class="main">
	...
	<form name="form1" action="<c:url value='/produce/order-make/index.jhtml'/>" method="post">
    ...
    <div class="ntable_mid mT5">
		<display:table name="pageList" id="vo" class="nm_table" style="width:100%" cellspacing="0" requestURI="index.jhtml">
			...
			<display:column title="等级" style="width:60px;">${vo.grade.text}</display:column>
			...
			<display:column title="生产状态" style="width:80px;">${vo.makeState.text}</display:column>
			<display:column title="操作" style="width:100px;">
				<span class="caozuo">
					<a href="#" onclick="openDialog('<c:url value="/sell/order/sell-order/details.jhtml?id=${vo.id}"/>','${vo.code }的订单详情');" class="show">详情</a>
					&nbsp;|&nbsp;<a href="edit.jhtml?id=${vo.id}" class="edit">生产</a>
				</span>
			</display:column>
			<display:setProperty name="paging.banner.item_name">订单生成</display:setProperty>
			<display:setProperty name="paging.banner.items_name">订单生成</display:setProperty>
		</display:table>
	</div>
