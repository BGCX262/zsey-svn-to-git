 ...
 * 
 */
public class ConfirmPriceAction extends OrderAction {
	/**
	 * 首页
	 */
	public String index(){
	...
	public String edit(){
		if(id!=null){
			...
			mindSO.setDataId(id);
			mindSO.setSort(OrderState.XSGL_BJGL_BJQR);
...
//		if (mindSO.getState().equals(StateType.PRICE_YES)) {
//			vo.setAskState(StateType.CONFIRM_YES);
//			vo.setState(OrderState.XSGL_BJGL_BJGL);
//		} else {
//			vo.setAuditState(StateType.AUDIT_WAIT);
//			vo.setAskState(StateType.PRICE_NO);
//			vo.setState(OrderState.XSGL_BJGL_CPBJ);
//		}
		productPriceDAO.saveOrUpdate(vo);
		
		return redirectSuccess("/sell/order/confirm-price/index.jhtml", "数据保存成功");
...
//	private AskType[] types=AskType.values();
	private StateType[] states={StateType.PRICE_YES,StateType.PRICE_NO};
	@Resource
